...
type MixedValue = 'mixed';

type SerializableColor = {
  r: number;
  g: number;
  b: number;
  a: number;
...
type SerializablePaint = {
  ...
  blendMode?: BlendMode;
  color?: SerializableColor;
  gradientStops?: SerializableGradientStop[];
...
type SerializableStyle = {
  ...
  fills?: SerializablePaint[] | MixedValue;
  strokes?: SerializablePaint[] | MixedValue;
  strokeWeight?: number | MixedValue;
  strokeAlign?: 'CENTER' | 'INSIDE' | 'OUTSIDE' | MixedValue;
  strokeCap?: StrokeCap | MixedValue;
  strokeJoin?: StrokeJoin | MixedValue;
  strokeDashes?: number[];
  cornerRadius?: number | MixedValue;
  cornerRadii?: number[];
...
type SerializableLayout = {
  ...
  sizingHorizontal?: LayoutMixin['layoutSizingHorizontal'];
  sizingVertical?: LayoutMixin['layoutSizingVertical'];
  primaryAxisSizing?: AutoLayoutMixin['primaryAxisSizingMode'];
  counterAxisSizing?: AutoLayoutMixin['counterAxisSizingMode'];
  primaryAxisAlign?: AutoLayoutMixin['primaryAxisAlignItems'];
  counterAxisAlign?: AutoLayoutMixin['counterAxisAlignItems'];
...
type SerializableLayoutItem = {
  align?: AutoLayoutChildrenMixin['layoutAlign'];
  grow?: AutoLayoutChildrenMixin['layoutGrow'];
  positioning?: AutoLayoutChildrenMixin['layoutPositioning'];
  sizingHorizontal?: LayoutMixin['layoutSizingHorizontal'];
  sizingVertical?: LayoutMixin['layoutSizingVertical'];
};

type SerializableTextMetric = {
  unit: string;
  value?: number;
};

type SerializableTextSegment = {
  start: number;
  end: number;
  text: string;
  fontFamily?: string;
  fontStyle?: string;
  fontSize?: number;
  lineHeight?: SerializableTextMetric;
  letterSpacing?: SerializableTextMetric;
  textCase?: TextCase;
...
type SerializableTextStyle = {
  fontFamily?: string | MixedValue;
  fontStyle?: string | MixedValue;
  fontSize?: number | MixedValue;
  lineHeight?: SerializableTextMetric | MixedValue;
  letterSpacing?: SerializableTextMetric | MixedValue;
  textCase?: TextCase | MixedValue;
  textDecoration?: TextDecoration | MixedValue;
  textAlignHorizontal?: TextNode['textAlignHorizontal'];
  textAlignVertical?: TextNode['textAlignVertical'];
  paragraphSpacing?: number;
  paragraphIndent?: number;
  segments?: SerializableTextSegment[];
};

type SerializableComponent = {
  kind: 'FRAME' | 'INSTANCE' | 'COMPONENT' | 'COMPONENT_SET' | 'VARIANT';
  name: string;
...
type ParsedNode = {
  ...
  layoutItem?: SerializableLayoutItem;
  textStyle?: SerializableTextStyle;
...
function serializeNumberOrMixed(value: number | PluginAPI['mixed'] | undefined): number | MixedValue | undefined {
...
function serializeEnumOrMixed<T extends string>(value: T | PluginAPI['mixed'] | undefined): T | MixedValue | undefined {
  if (typeof value === 'undefined') {
    return undefined;
  }
...
function serializeColor(color: RGB | RGBA, opacity = 1): SerializableColor {
...
): SerializablePaint[] | MixedValue | undefined {
...
function serializeEffect(effect: Effect): SerializableEffect {
  const serialized: SerializableEffect = {
    type: effect.type,
...
function getNodeStyle(node: SceneNode): SerializableStyle | undefined {
...
function getNodeLayout(node: SceneNode): SerializableLayout | undefined {
...
function getNodeLayoutItem(node: SceneNode): SerializableLayoutItem | undefined {
  if (!('layoutAlign' in node)) {
    return undefined;
  }

  const layoutItem: SerializableLayoutItem = {
...
function serializeTextMetric(metric: LetterSpacing | LineHeight | PluginAPI['mixed']): SerializableTextMetric | MixedValue {
  if (isMixed(metric)) {
    return 'mixed';
  }

  const serialized: SerializableTextMetric = {
    unit: metric.unit,
  };
...
function serializeFontName(fontName: FontName | PluginAPI['mixed']): {
  fontFamily?: string | MixedValue;
  fontStyle?: string | MixedValue;
} {
  if (isMixed(fontName)) {
...
function getTextSegments(node: TextNode): SerializableTextSegment[] | undefined {
...
function getNodeTextStyle(node: SceneNode): SerializableTextStyle | undefined {
  if (node.type !== 'TEXT') {
    return undefined;
  }

  const fontInfo = serializeFontName(node.fontName);
  const textStyle: SerializableTextStyle = {
...
function readVariantProperties(node: InstanceNode | ComponentNode): Record<string, string> | undefined {
  try {
...
function readComponentPropertyReferences(node: SceneNode): Record<string, string> | undefined {
...
function getNodeComponent(node: SceneNode): SerializableComponent | undefined {
